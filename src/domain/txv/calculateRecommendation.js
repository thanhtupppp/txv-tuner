...
import { TXV_VALIDATION_CODES } from './validateTxvInput';

/**
 * Pure calculation module xác định hướng dẫn và số vòng vặn vít van tiết lưu Danfoss TXV.
...
export function calculateTxvRecommendation(actualSh, targetSh, valve, interlock = { allowed: true }) {
  ...
  if (!valve || typeof valve.sensitivity !== 'number' || valve.sensitivity <= 0) {
    return {
      allowed: false,
      value: null,
      turns: null,
      turnsFraction: null,
      direction: 'NONE',
      text: 'Không có dữ liệu van hoặc độ nhạy van không hợp lệ. Vui lòng chọn dòng van Danfoss.',
      status: 'error',
  ...
  // 4. Kiểm tra targetSh
  if (typeof targetSh !== 'number' || !Number.isFinite(targetSh) || targetSh <= 0) {
    return {
      allowed: false,
      value: null,
      turns: null,
      turnsFraction: null,
      direction: 'NONE',
      text: 'Quá nhiệt mục tiêu (Target SH) không hợp lệ.',
      status: 'invalid_input',
      statusText: 'MỤC TIÊU SH KHÔNG HỢP LỆ',
      reason: 'INVALID_TARGET_SH',
      heading: 'Mục tiêu SH không hợp lệ',
    };
  }

  // 5. Chốt an toàn: Heuristic phát hiện bất thường nhiệt động (SH < -5.0K)
  if (actualSh < -5.0) {
    return {
      allowed: false,
      value: actualSh,
